...
export interface OptionWheelProps {
  ...
  onOptionClick?: (index: number, item: string) => void;
  textColor?: string;
  activeColor?: string;
  side?: 'left' | 'right';
  fontSize?: number;
...
export const OptionWheel: React.FC<OptionWheelProps> = ({
  ...
  // Pure layout update function that places each item along the mathematical curve
  ...
  // Wheel / touchpad scrolling listener
  useEffect(() => {
  ...
  const handleItemClick = useCallback(
    (index: number) => {
      ...
      onOptionClick?.(index, cfg.items[index]);
    },
    [applyTarget, onOptionClick]
  );
  ...
  // Guarantee layout update on mount and prop changes
  useEffect(() => {
    updateLayout();
    const id = requestAnimationFrame(() => updateLayout());
    const id2 = setTimeout(() => updateLayout(), 60);
    return () => {
      cancelAnimationFrame(id);
      clearTimeout(id2);
    };
  }, [updateLayout, items, fontSize, spacing, curve, tilt, blur, fade, minOpacity, side, loop, smoothing]);
